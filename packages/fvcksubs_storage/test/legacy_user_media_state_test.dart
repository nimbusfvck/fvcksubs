...
void main() {
  ...
  group('LegacyUserMediaState', () {
    test('round-trips a bare (never-watched, not favorited) record', () {
      final state = LegacyUserMediaState(ref: item().ref, item: item());
      final decoded = LegacyUserMediaState.fromJson(
    ...
    test('round-trips a favorited, in-progress record', () {
      final state = LegacyUserMediaState(
        ref: item().ref,
        item: item(),
        favorite: true,
    ...
    test('keyFor includes extension and provider, not just the opaque id', () {
      ...
      expect(
        LegacyUserMediaState.keyFor(refA),
        isNot(LegacyUserMediaState.keyFor(refB)),
      );
    });
