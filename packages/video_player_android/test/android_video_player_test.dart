  ...
  group('AndroidVideoPlayer', () {
    ...
    test('create with network passes Widevine DRM configuration', () async {
      ...
      final PlatformWidevineDrmConfiguration? widevineDrm =
          creationOptions.widevineDrm;
      expect(widevineDrm, isNotNull);
      expect(widevineDrm!.licenseUri, 'https://example.com/license');
      expect(widevineDrm.licenseHeaders, <String, String>{
        'Authorization': 'Bearer token',
      });
    });

    test('create with network passes ClearKey DRM configuration', () async {
      final (AndroidVideoPlayer player, MockAndroidVideoPlayerApi api, _) =
          setUpMockPlayer(playerId: 1, textureId: 100);
      when(
        api.createForTextureView(any),
      ...
      await player.create(
        DataSource(
          sourceType: DataSourceType.network,
          uri: 'https://example.com/video.mpd',
          drmConfiguration: const ClearKeyDrmConfiguration(
            clearKeyJson: '{"keys":[]}',
          ),
        ),
      );
      ...
      final PlatformClearKeyDrmConfiguration? clearKeyDrm =
          creationOptions.clearKeyDrm;
      expect(clearKeyDrm, isNotNull);
      expect(clearKeyDrm!.clearKeyJson, '{"keys":[]}');
      expect(creationOptions.widevineDrm, isNull);
    });
