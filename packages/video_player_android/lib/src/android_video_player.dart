...
import 'clear_key_drm_configuration.dart';
...
class AndroidVideoPlayer extends VideoPlayerPlatform {
  ...
  Future<int?> createWithOptions(VideoCreationOptions options) async {
    ...
    final PlatformWidevineDrmConfiguration? widevineDrm =
  ...
  // either because it isn't a supported Android DRM configuration or because
  // the source isn't a network source.
  PlatformWidevineDrmConfiguration? _widevineDrmFromDataSource(
    DataSource dataSource,
  ) {
    ...
    if (drmConfiguration is ClearKeyDrmConfiguration) {
      return null;
    }
    ...
    if (drmConfiguration is! WidevineDrmConfiguration) {
      throw ArgumentError.value(
        ...
        'The Android implementation of video_player only supports '
            'WidevineDrmConfiguration and ClearKeyDrmConfiguration',
      );
    }
  ...
  // Returns the Pigeon representation of the ClearKey configuration in
  // [dataSource], or null if it has none.
  PlatformClearKeyDrmConfiguration? _clearKeyDrmFromDataSource(
    DataSource dataSource,
  ) {
    final VideoDrmConfiguration? drmConfiguration = dataSource.drmConfiguration;
    if (drmConfiguration == null) {
    ...
    return PlatformClearKeyDrmConfiguration(
      clearKeyJson: drmConfiguration.clearKeyJson,
    );
  }
