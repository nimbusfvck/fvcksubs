...
static NSDictionary<NSString *, NSValue *> *FVPGetPlayerItemObservations(void) {
  return @{
    ...
    @"seekableTimeRanges" : [NSValue valueWithPointer:timeRangeContext],
...
- (void)observeValueForKeyPath:(NSString *)path
  ...
  if (context == timeRangeContext) {
    ...
    for (NSValue *rangeValue in [(AVPlayerItem *)object loadedTimeRanges]) {
    ...
    NSMutableArray<NSArray<NSNumber *> *> *seekableValues = [[NSMutableArray alloc] init];
    for (NSValue *rangeValue in [(AVPlayerItem *)object seekableTimeRanges]) {
      CMTimeRange range = [rangeValue CMTimeRangeValue];
      [seekableValues addObject:@[
        @(FVPCMTimeToMillis(range.start)),
        @(FVPCMTimeToMillis(range.duration)),
      ]];
    }
