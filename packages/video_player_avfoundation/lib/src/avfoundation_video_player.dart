...
class AVFoundationVideoPlayer extends VideoPlayerPlatform {
  ...
  PlatformFairPlayDrmConfiguration? _fairPlayDrmFromDataSource(
    DataSource dataSource,
  ) {
    final VideoDrmConfiguration? drmConfiguration = dataSource.drmConfiguration;
    if (drmConfiguration == null) return null;
...
class _PlayerInstance {
  ...
  void _onStreamEvent(dynamic event) {
    ...
    _eventStreamController.add(switch (map['event']) {
      ...
      'bufferingUpdate' => VideoEvent(
        buffered: _toDurationRanges(map['values']),
        seekable: _toDurationRanges(map['seekableValues']),
  ...
  List<DurationRange>? _toDurationRanges(dynamic value) {
    if (value is! List<dynamic>) return null;
    return value.map<DurationRange>(_toDurationRange).toList();
  }
}
