...
void main() {
  ...
  test('toggleFavorite emits and persists the updated records', () async {
    ...
    final controller = LibraryControllerV2(store: store);
  ...
  test('watching can explicitly clear progress', () {
    final controller = LibraryControllerV2(store: _MemoryLibraryStoreV2());
    controller.recordWatched(item(), progress: const Duration(minutes: 12));
  ...
  test('initial records are exposed as immutable state', () {
    final record = UserMediaStateV2(item: item(), favorite: true);
...
class _MemoryLibraryStoreV2 implements LibraryStoreV2 {
  Map<String, UserMediaStateV2> records = {};

  @override
  Future<Map<String, UserMediaStateV2>> load() async => Map.of(records);
