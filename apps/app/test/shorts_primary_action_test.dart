...
void main() {
  test('preview-only football items do not expose full playback', () {
    const ucl = VideoItemV2(
      ref: _ref,
      title: 'UCL preview',
      tags: ['sports', 'football', 'champions-league'],
    );
    const premierLeague = VideoItemV2(
      ref: _ref,
      title: 'Premier League preview',
      tags: ['sports', 'football', 'premier-league'],
    );
    const movie = VideoItemV2(ref: _ref, title: 'Movie', tags: ['movie']);

    expect(shortsHasFullPlayback(ucl), isFalse);
    expect(shortsHasFullPlayback(premierLeague), isFalse);
    expect(shortsHasFullPlayback(movie), isTrue);
  });
  ...
  test(
    'an available standalone video shows Watch, then Continue once in progress',
    () {
      const item = VideoItemV2(ref: _ref, title: 't');

      final fresh = primaryActionFor(
        item,
        detail: null,
        library: LibraryState(),
      );
  ...
  test(
    'a series with episodes announced but none released yet shows Details',
    () {
      const series = SeriesItemV2(ref: _ref, title: 't');
  ...
  test(
    'a series with a resolvable episode shows Watch, or Continue while resuming',
    () {
      const episodeRef = MediaRef(
        extensionId: 'fake',
        providerId: 'fake.p',
      ...
      const guide = EpisodeGuide(
        groups: [
          EpisodeGroup(
            ...
            title: 'Season 1',
            episodes: [
              EpisodeSummary(ref: episodeRef, title: 'e', position: 1),
            ],
          ),
        ],
      );
      ...
      final resuming = primaryActionFor(
        series,
        detail: detail,
        library: resumingLibrary,
      );
