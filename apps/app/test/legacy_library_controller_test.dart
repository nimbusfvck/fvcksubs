...
void main() {
  ...
  test('toggleFavorite flips the flag, persists, and notifies', () async {
    ...
    final controller = LegacyLibraryController(store: store);
    var notified = 0;
  ...
  test(
    'un-favoriting an item with no watch history drops the record',
    () async {
  ...
  test('history is sorted most-recently-watched first', () {
    ...
    expect(controller.history.map((s) => s.ref.id).toList(), [
      'newer',
      'older',
    ]);
  });
  ...
  test('favorites are sorted alphabetically by title', () {
    ...
    expect(controller.favorites.map((s) => s.item.title).toList(), [
      'Apple',
      'Zebra',
    ]);
  });
  ...
  test('starts seeded from initial records', () {
    final seed = LegacyUserMediaState(
      ref: item().ref,
      item: item(),
      favorite: true,
    );
    final controller = LegacyLibraryController(
